    ...
    <Collapsible className="space-y-1">
      ...
      <CollapsibleContent className="pl-8 space-y-1">
        {listItem.subList.map((sub) => (
          <Link
            key={sub.id}
            className="flex items-center gap-3 rounded-md px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-200 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-gray-800 dark:hover:text-gray-50"
            href={sub.link}
          >
            {sub.item}
          </Link>
